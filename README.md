...
**A device's data can be shaped.** The **Data** tab gives a model its size where it has one — the ports of a Catalyst, the disks of the Synology, the outlets of the rack PDU, the processors of the Linux server — and takes values of the device's own, OID by OID, in the types a model file uses: each takes the place of the model's value at its OID, or adds an instance the model does not have. A **preview** below shows what the device would answer the moment it starts, from the settings as they are on screen, saved or not, each OID named from the loaded MIBs. The **Faults** tab applies faults from the editor, as the list's button does.
