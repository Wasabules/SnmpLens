  ...
  for (const t of targets) {
    ...
    for (let at = start; at <= end; at += intervalMs) {
      ...
      if (t.story === 'climbing' && progress > 0.62) {
        // Crosses the threshold and stays over it: an incident, not a blip.
        value += (progress - 0.62) * 120;
      }
      if (t.story === 'spike') {
      ...
      if (t.story === 'silent' && progress > 0.78 && progress < 0.86) {
      ...
      value = Math.max(0, Math.min(100, value + (rand() - 0.5) * 3.2));
