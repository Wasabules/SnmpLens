...
func TestTheRendererDistinguishesAFailedCheck(t *testing.T) {
	for _, f := range []string{
		filepath.Join(repoRoot, "frontend", "src", "settings", "GeneralSettings.svelte"),
		filepath.Join(repoRoot, "frontend", "src", "stores", "updateStore.js"),
	} {
	...
	body, err := os.ReadFile(filepath.Join(repoRoot, "frontend", "src", "settings", "GeneralSettings.svelte"))
	if err != nil {
		t.Fatal(err)
	}
